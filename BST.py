  ...
  def remove_node(self, cur_node, rem_node, prev_node = None):
    ...
    if rem_node == cur_node.data:
      ...
      #rem_node has two children
        ...
        self.nodes = []
        self.traverse_tree(cur_node) # get list of all decendants
        self.nodes.remove(rem_node) #remove node we are removing from list
        self.contents.remove(rem_node)
        # remove node and all decendants
        if cur_node == prev_node.right:
          prev_node.right = None
        else: 
          prev_node.left = None
        # add all decendants back into tree
    ...
    #wrong node, traverse
      ...
      self.remove_node(cur_node, rem_node, prev_node = cur_node)
